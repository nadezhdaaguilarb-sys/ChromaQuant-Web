...
# ==========================================
st.set_page_config(
    page_title="ChromaQuant",
    page_icon="🔬",
    layout="wide"
)

# ==========================================
# FIX PARA MOSTRAR LA IMAGEN DE FONDO DEL
# CANVAS EN STREAMLIT COMMUNITY CLOUD
# ==========================================
_original_canvas_image_to_url = sdc.image_to_url

def canvas_image_to_data_url(image, *args, **kwargs):
    ...
    return _original_canvas_image_to_url(image, *args, **kwargs)

sdc.image_to_url = canvas_image_to_data_url
...
def tissue_mask(img_hsv, s_min=TISSUE_S_MIN, v_min=TISSUE_V_MIN):
    ...
    return cv2.bitwise_and(
        cv2.inRange(s, s_min, 255),
        cv2.inRange(v, v_min, 255)
    )
...
def compute_exclusive_color_masks_and_stats(img_hsv, roi_mask=None):
    ...
    no_clasif = cv2.bitwise_and(
        den,
        cv2.bitwise_not(assigned)
    )

    masks_exclusive["no_clasif"] = no_clasif
    counts["no_clasif"] = int(
        cv2.countNonZero(no_clasif)
    )
...
def login():
    ...
    if st.button("Verificar Acceso"):
        if email:
            query = (
                db.collection("usuarios_permitidos")
                .where(
                    "correo",
                    "==",
                    email.strip()
...
def main_app():
    ...
    if archivo:
        ...
        img_bgr_original = cv2.imdecode(
            file_bytes,
            1
        )

        h_orig, w_orig = (
            img_bgr_original.shape[:2]
        )

        # Imagen reducida SOLO para
        ...
        img_bgr_web = cv2.resize(
            img_bgr_original,
            (nw, nh)
        )

        img_rgb_web = cv2.cvtColor(
            img_bgr_web,
            cv2.COLOR_BGR2RGB
        )

        img_para_lienzo = Image.fromarray(
            img_rgb_web
        ).convert("RGB")
        ...
        modo_dibujo = st.sidebar.radio(
            "Tipo de selección:",
            options=list(
                opciones_dibujo.keys()
            ),
        ...
        color_ver = st.sidebar.selectbox(
        ...
        # 1. Obtener la máscara de la web
        # y ESCALARLA a resolución original
        roi_mask_orig = None
        ...
        if (
            ...
            if np.any(alfa > 0):
                ...
                # para hacer cálculos reales.
                # INTER_NEAREST mantiene los
                # bordes de la máscara.
                roi_mask_orig = cv2.resize(
                    roi_mask_web,
                    (w_orig, h_orig),
                    interpolation=(
                        cv2.INTER_NEAREST
        ...
        # 2. Hacer las matemáticas sobre
        # la imagen ORIGINAL
        with st.spinner(
            "Calculando píxeles en "
            "resolución original..."
        ):
            img_hsv_orig = cv2.cvtColor(
                img_bgr_original,
                cv2.COLOR_BGR2HSV
            )

            masks_orig, counts, total = (
                compute_exclusive_color_masks_and_stats(
                    img_hsv_orig,
                    roi_mask_orig
                )
            )
        ...
        with col2:
            ...
            if color_ver == "todo":
                ...
                mask_show_orig = (
                    masks_orig.get(color_ver)
                )

                mask_show_web = cv2.resize(
                    mask_show_orig,
                    (nw, nh),
                    interpolation=(
                        cv2.INTER_NEAREST
        ...
        if total > 0:
            ...
            p_n = (
                counts["naranja"] / total
            ) * 100

            p_a = (
                counts["amarillo"] / total
            ) * 100
            ...
            m5.metric(
                "No Clasif.",
                f"{p_nc:.2f}%",
                f"{counts['no_clasif']:,} px"
            )

            # ----------------------------------
            # GENERAR ARCHIVO ZIP CON RECORTE
            # ----------------------------------
...
"""
            ...
            st.warning(
                "No hay tejido detectado "
                "o no has dibujado un "
                "ROI válido."
            )
